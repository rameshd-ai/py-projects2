...
PAYLOAD_LOG_FILE = os.path.join("output", "api_payload_log.jsonl")
def addUpdateRecordsToCMS(base_url, headers, payload):
    ...
    """
    Calls the CMS API to save/update miBlock records using the provided payload.
    Logs each single-record payload to PAYLOAD_LOG_FILE in JSONL format before sending.
    ...
    # 1. Ensure the log file directory exists (once per call)
    try:
        os.makedirs(os.path.dirname(PAYLOAD_LOG_FILE), exist_ok=True)
    except Exception as e:
        # Handle cases where os.path.dirname(PAYLOAD_LOG_FILE) might be "." or invalid
        pass 
    ...
    try:
        for record_set_id, records in payload.items():
            for record in records:
                ...
                # 2. LOG THE PAYLOAD FOR DEBUGGING (JSONL format)
                try:
                    # Use 'a' (append mode) to add the new payload (NDJSON format).
                    with open(PAYLOAD_LOG_FILE, "a", encoding='utf-8') as f:
                        # Add metadata for context, like the level key
                        log_entry = {
                            "timestamp": int(time.time() * 1000), 
                            "level_key": record_set_id,
                            "payload": record
                        }
                        json.dump(log_entry, f)
                        f.write('\n')
                    # print(f"💾 Logged payload for {record_set_id} to {PAYLOAD_LOG_FILE}")
                except Exception as log_err:
                    print(f"[WARN] Warning: Could not save payload to log file {PAYLOAD_LOG_FILE}. Error: {log_err}")
                ...
                if result.get("result"):
                    # We use the recordId from the outgoing payload as the key for the response map
                    responses[record_id] = result.get('result') 
                else:
                    return False, f"API response indicates failure for record: {record}"
        ...
        # You might want to log the specific request that failed here too, 
        # but the JSONL file already contains the outgoing payload.
        return False, f"Failed to update records via API: {e}"
    except Exception as e:
        return False, f"An unexpected error occurred in API call: {e}"
...
PAYLOAD_LOG_FILE = os.path.join("output", "api_payload_log.jsonl")
def CreateComponentRecord(base_url: str, headers: Dict[str, str], payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Sends a POST request to create a component record and logs the payload before sending.

    Args:
        base_url: The root URL for the API (e.g., "https://example.com").
        headers: HTTP headers (including Authorization).
        payload: The data payload for the component record.

    Returns:
        The JSON response body from the API call, or an error dictionary.
    """
    ...
    # 2. Construct the final API endpoint URL
    api_url = f"{base_url}/api/MiblockApi/CreateComponentRecord"
    ...
    try:
        ...
        # Note: You can increase 'timeout' here if the 401/other errors disappear but the script hangs.
        response = requests.post(
            api_url,
            headers=headers,
            json=payload,
        ...
        # 4. Raise an exception for bad status codes (4xx or 5xx)
        response.raise_for_status()

        # 5. Return the successful JSON response content
        return response.json()

    except requests.exceptions.HTTPError as http_err:
        # Attempt to include API response body on error for better debugging (e.g., for 400 errors)
        error_info = {"response_text": response.text}
        try:
            error_info = response.json()
        except json.JSONDecodeError:
            pass # Use response_text if JSON decoding fails
        ...
        print(f"[ERROR] Failed to decode JSON response. Response text: {response.text}")
        return {"error": "JSON Decode Error", "details": "Response was not valid JSON", "response_text": response.text}