...
#             print(f"[OK] Created component name map and saved to: {map_file_path}")
#         else:
#             logging.warning(f"[WARN] {config_file_name} not found at {config_file_path}. Cannot create component map.")
...
def update_display_orders_in_payload(file_path):
    ...
    if not data:
        logging.error(f"[ERROR] Failed to load data from {file_path} for display order update.")
        return False
    ...
    try:
        ...
        print(f"[OK] Display orders successfully updated and saved to {file_path}.")
        return True
        
    except Exception as e:
        logging.error(f"[ERROR] Error during display order update/save: {e}")
        return False
...
#                 add_menu_data()
#             else:
#                  logging.warning(f"[WARN] Failed to load data from {config_file_name}.")
#         else:
#             logging.warning(f"[WARN] {config_file_name} not found at {config_file_path}. Cannot create component map.")
...
#         logging.error(f"[ERROR] Cannot update payload: Component map not found at {map_file_path}.")
#         return None 
...
#         logging.error("[ERROR] Cannot update payload: Main 'Menu' ComponentId not found in map.")
#         return None
...
#         logging.error(f"[ERROR] Cannot update payload: Target file {payload_file_name} not found or invalid at {payload_file_path}.")
#         return None
...
#         print(f"[OK] Successfully updated and saved {payload_file_name} to {payload_file_path}")
        
#         # Return the folder name
#         return mi_block_folder 

#     except IOError as e:
#         logging.error(f"[ERROR] Error saving final updated payload to {payload_file_path}: {e}")
#         return None
...
def preprocess_menu_data(payload: Dict, token: str) -> str | None:
    ...
    try:
            ...
            try:
                if extracted_file.endswith('.txt'):
                    ...
                    if not content.strip():
                        logging.warning(f"[WARN] Skipping empty file: {extracted_file}")
                        try:
                            os.remove(extracted_file_path)
                        except OSError:
                            pass
                ...
                logging.error(f"[ERROR] JSON Decode Error in file: {extracted_file_path}. Error: {e}")
                logging.error(f"[ERROR] Invalid content start (first 100 chars): '{content[:100].replace(chr(10), ' ')}...'")
                # Skip the bad file to allow the rest of the script to run
                continue 
                
            except OSError as e:
                # Catch OS-level errors
                logging.error(f"[WARN] OS Error processing file {extracted_file_path}: {e}")
                continue
        ...
        logging.error(f"[ERROR] UNEXPECTED MAJOR ERROR during file processing/mapping: {e}", exc_info=True)
        return None
...
#     print(f"[OK] Successfully loaded payload with {len(final_payload)} top-level menu records.")
...
#         print(f"\n[OK] DEBUG SUCCESS: ONLY API Responses for {len(processed_output_level_3a)} L3A records saved to: {final_api_output_file_path}")
#         return True
#     except Exception as e:
#         print(f"[ERROR] Error during L3A save: {e}")
#         return False
    ...
    for i, item_price_record in enumerate(final_item_prices_payloads):
        ...
        if destination_site_url and destination_token:
            ...
            if isinstance(api_result, dict):
                ...
                status_code = responseData.get('ErrorCode', 'N/A')
                success_status = responseData.get('Success', 'N/A')
                print(f"  [API] API Response Status: {status_code} | Success: {success_status}")
                # --- END FIX ---
                
            else:
                responseData = {"Note": "API call failed (returned non-dict/None).", "status_code": "ERROR_NON_DICT", "original_response": str(api_result)}
                print("  [ERROR] API call failed to return a valid dictionary response.")
        else:
            responseData = {"Note": "API call skipped due to missing settings.", "prepared_payload_example": fixed_payload}
...
#     if not final_payload:
#         print(f"[ERROR] Failed to load final payload from {payload_file_path}. Aborting.")
#         return False
...
#         print(f"[OK] DEBUG SUCCESS: ONLY API Responses for {len(processed_output_level_3b)} L3B records saved to: {final_api_output_file_path}")
#         return True
#     except (IOError, TypeError) as e:
#         print(f"[ERROR] Error saving final processed output to {final_api_output_file_path}: {e}")
#         return False
