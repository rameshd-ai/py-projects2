...
def fix_component_ids_in_payload(file_prefix: str, downloaded_component_id: int, site_id: int) -> bool:
    """
    Fixes componentId for records in payloads by mapping them to the correct child component based on level.
    - Level 1 records should have the Level 1 componentId
    - Level 2 records should have the Level 2 componentId
    
    Args:
        file_prefix: The file prefix for the payload files
        downloaded_component_id: The main parent component ID
        site_id: The site ID
    ...
    try:
        ...
        if not os.path.exists(config_file_path):
            logging.error(f"Component config file not found: {config_file_path}")
            return False
        ...
        if not components:
            logging.error("No components found in config file")
            return False
        
        # Build level-to-componentId mapping
        # Level 0 = parent component (container)
        # Level 1 = first child (main menu items)
        # Level 2 = second child (sub-menu items)
        component_id_map = {}
        
        # Find parent component
        parent_component = next((c for c in components if c.get("ComponentId") == downloaded_component_id), None)
        if parent_component:
            component_id_map[0] = downloaded_component_id
            logging.info(f"Level 0 (Container): ComponentId={downloaded_component_id}")
        
        # Find child components by ParentId
        level_1_component = next((c for c in components if c.get("ParentId") == downloaded_component_id), None)
        if level_1_component:
            level_1_id = level_1_component.get("ComponentId")
            component_id_map[1] = level_1_id
            logging.info(f"Level 1: ComponentId={level_1_id}, Name={level_1_component.get('ComponentName')}")
            
            # Find level 2 component (child of level 1)
            level_2_component = next((c for c in components if c.get("ParentId") == level_1_id), None)
            if level_2_component:
        ...
        logging.info(f"Component ID mapping by level: {component_id_map}")
        
        # Build property alias mappings for each component
        component_definitions = config_data.get("componentDefinition", [])
        property_alias_map = {}
        
        for comp_id in component_id_map.values():
            # Get property aliases for this component
            comp_defs = [d for d in component_definitions if d.get("ComponentId") == comp_id]
            if comp_defs:
                # Find name and link properties
                name_prop = None
                link_prop = None
                
                for prop_def in comp_defs:
                    alias = prop_def.get("PropertyAliasName", "")
                    is_identifier = prop_def.get("IsIdentifier", False)
                    
                    # Find the name property (IsIdentifier=true)
                    if is_identifier:
                        name_prop = alias
                    
                    # Find the link property (contains 'link' in alias)
                    if 'link' in alias.lower() and 'item' in alias.lower():
                        link_prop = alias
                
                if name_prop and link_prop:
                    property_alias_map[comp_id] = {
                        'name_key': name_prop,
                        'link_key': link_prop
                    }
                    logging.info(f"ComponentId {comp_id}: name_key='{name_prop}', link_key='{link_prop}'")
        
        if not property_alias_map:
            logging.error("Could not build property alias mapping")
            return False
        
        # Fix componentId in new_records_payload.json
        ...
        if os.path.exists(new_records_file):
            ...
            for record in new_data.get("records", []):
                ...
                if rec_level in component_id_map:
                    correct_component_id = component_id_map[rec_level]
                    if old_component_id != correct_component_id:
                        record["componentId"] = correct_component_id
                        updated_count += 1
                        logging.info(f"[FIX] '{page_name}' (level {rec_level}): componentId {old_component_id} → {correct_component_id}")
                    
                    # Fix property names in recordDataJson
                    if correct_component_id in property_alias_map:
                        try:
                            record_json_string = record.get("recordDataJson", "{}")
                            record_data = json.loads(record_json_string)
                            
                            correct_name_key = property_alias_map[correct_component_id]['name_key']
                            correct_link_key = property_alias_map[correct_component_id]['link_key']
                            
                            # Find current name and link keys (they might be page-specific)
                            ...
                            for key, value in list(record_data.items()):
                                if key.endswith('-name') and key not in ['Id', 'ParentId']:
                                    current_name_key = key
                                    name_value = value
                                elif key.endswith('-link') and key not in ['Id', 'ParentId']:
                                    current_link_key = key
                                    link_value = value
                            
                            # Replace with correct keys
                            fixed = False
                            if current_name_key and current_name_key != correct_name_key:
                                del record_data[current_name_key]
                                record_data[correct_name_key] = name_value
                                fixed = True
                                logging.info(f"  [FIX] '{page_name}': name key '{current_name_key}' → '{correct_name_key}'")
                            
                            if current_link_key and current_link_key != correct_link_key:
                                del record_data[current_link_key]
                                record_data[correct_link_key] = link_value
                                fixed = True
                                logging.info(f"  [FIX] '{page_name}': link key '{current_link_key}' → '{correct_link_key}'")
                            
                            if fixed:
                                record["recordDataJson"] = json.dumps(record_data, ensure_ascii=False)
                                property_fixed_count += 1
                        
                        except Exception as e:
                            logging.error(f"Error fixing properties for '{page_name}': {e}")
            
            if updated_count > 0 or property_fixed_count > 0:
                with open(new_records_file, 'w', encoding='utf-8') as f:
                    json.dump(new_data, f, indent=4, ensure_ascii=False)
                if updated_count > 0:
                    logging.info(f"[SUCCESS] Fixed {updated_count} componentId values in new_records_payload.json")
                if property_fixed_count > 0:
                    logging.info(f"[SUCCESS] Fixed {property_fixed_count} property names in new_records_payload.json")
        
        # Fix componentId in save_miblock_records_payload.json
        ...
        if os.path.exists(matched_records_file):
            ...
            if updated_count > 0 or property_fixed_count > 0:
                ...
                if updated_count > 0:
                    logging.info(f"[SUCCESS] Fixed {updated_count} componentId values in save_miblock_records_payload.json")
                if property_fixed_count > 0:
                    logging.info(f"[SUCCESS] Fixed {property_fixed_count} property names in save_miblock_records_payload.json")
        
        return True
        
    except Exception as e:
        logging.error(f"Error fixing component IDs: {e}")
                ...
                if level_1_custom_properties:
                    ...
                    if os.path.exists(matched_records_file):
                        ...
                        if updated_count > 0:
                            ...
                            if api_base_url and api_headers:
                                # Fix componentId values before API call
                                logging.info("Fixing componentId values in matched records payload...")
                                fix_component_ids_in_payload(file_prefix, downloaded_component_id, site_id)
                                ...
                                # Save/update all payloads to debug file
                                debug_file = os.path.join(UPLOAD_FOLDER, f"{file_prefix}_all_api_payloads.json")
                                all_payloads = {}
                                ...
                                all_payloads["matched_records_payload"] = matched_records
                                all_payloads["matched_records_count"] = len(matched_records)
                                
                                with open(debug_file, 'w', encoding='utf-8') as f:
                                    json.dump(all_payloads, f, indent=4, ensure_ascii=False)
                                logging.info(f"[DEBUG] Updated all API payloads file: {debug_file}")
                    ...
                    if api_base_url and api_headers:
                        # Fix componentId values before API calls
                        logging.info("Fixing componentId values in payloads...")
                        fix_component_ids_in_payload(file_prefix, downloaded_component_id, site_id)
