...
def execute_single_step(job_id: str, step_number: int) -> Dict[str, Any]:
    ...
    print(f"\n[EXECUTE] Executing step function: {step_function_name}", flush=True)
    print(f"[EXECUTE] Job ID: {job_id}", flush=True)
    ...
    print(f"\n[OK] Step function completed: {step_function_name}\n", flush=True)
        ...
        for idx, step_config in enumerate(PROCESSING_STEPS, 1):
            ...
            try:
                ...
                yield format_sse({
                    ...
                    "step_id": step_id,
                    "step_name": step_name,
                    "message": f"[OK] Completed: {step_name}"
                })
