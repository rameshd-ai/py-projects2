...
def run_site_setup_step(job_id: str, step_config: Dict, workflow_context: Dict) -> Dict[str, Any]:
    ...
    if source_url and source_profile_alias:
        try:
            ...
            if not cms_response_data:
                ...
                if source_token:
                    print(f"[OK] Source token generated successfully!", flush=True)
                    logger.info(f"[{job_id}] [OK] Source CMS token generated successfully")
                else:
        ...
        try:
            ...
            if not cms_response_data:
                ...
                if destination_token:
                    print(f"[OK] Destination token generated successfully!", flush=True)
                    logger.info(f"[{job_id}] [OK] Destination CMS token generated successfully")
                else:
    ...
    # NOTE: Theme API calls (get_theme_configuration, get_group_record) are handled in Step 2 (brand_theme.py)
    # They should NOT be called here in Step 1 (site_setup.py)
