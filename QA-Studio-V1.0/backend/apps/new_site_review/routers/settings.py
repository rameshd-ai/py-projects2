...
def get_ga4():
    ...
    if not s:
        return jsonify({"configured": False, "ga4_property_id": ""})
...
def put_ga4():
    ...
    s = save_ga4_settings(ga4_property_id=ga4_property_id, workspace_id=WORKSPACE_ID)
    return jsonify(_settings_to_response(s))


@settings_bp.route("/ga4/test", methods=["POST"])
def test_ga4():
    """Validate GA4 Property ID format; real connection requires OAuth login."""
    ...
    return jsonify({"success": True, "message": "Property ID valid. Sign in with Google (Play button) to fetch data."})
