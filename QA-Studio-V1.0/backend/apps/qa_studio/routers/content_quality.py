"""Content Quality scan API."""
from flask import Blueprint, request, jsonify

from apps.qa_studio.db.scan_history import save_content_quality_scan, load_content_quality_scan
from apps.qa_studio.services.content_quality import run_content_quality_scan, report_to_dict
from apps.qa_studio.services.sitemap import fetch_sitemap_urls

content_quality_bp = Blueprint("content_quality", __name__)


@content_quality_bp.route("/sitemap-urls", methods=["GET"])
...
@content_quality_bp.route("/scan", methods=["POST"])
def scan_url():
...
@content_quality_bp.route("/save-report", methods=["POST"])
def save_report():
    data = request.get_json()
    if not data:
        return jsonify({"detail": "JSON body required"}), 400
    ...
    try:
        save_content_quality_scan(project_id, {
            "urls": data.get("urls", []),
            "results": data.get("results", {}),
            "aggregate": data.get("aggregate", {}),
            "sitemap_source": data.get("sitemap_source", ""),
