(function () {
  ...
  // URL: /project/<projectId>/pillar/<pillarSlug>  => ['project', uuid, 'pillar', slug]
  const projectId = pathParts.length >= 4 && pathParts[0] === 'project' && pathParts[2] === 'pillar' ? pathParts[1] : null;
  const pillarSlug = pathParts.length >= 4 ? pathParts[3] : null;
  ...
  fetch(API + '/projects/' + encodeURIComponent(projectId) + '/pillars/' + encodeURIComponent(pillarSlug))
    ...
    .then(function (data) {
      ...
      el('pillar-back-link').href = '/project/' + encodeURIComponent(projectId);
