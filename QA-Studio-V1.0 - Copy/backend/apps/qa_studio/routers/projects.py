...
def _pillars_for_project(project):
    ...
    for i, (name, delta) in enumerate(names):
        ...
        pillars.append({
            ...
            "total": total, "last_run": last_run, "issues": _mock_issues(slug, failed, project.name),
        })
    return pillars
...
def create_project():
    ...
    project = project_store.create(name=payload.name, domain_url=payload.domain_url, phase=payload.phase)
