"""
Volume Dry-Up Breakout: low participation then sudden volume expansion.
"""
...
class VolumeDryUpBreakout(BaseStrategy):
    """Dry-up (volume < 0.5× avg) then expansion (> 1.5× avg) on last candle."""
    ...
    def check_entry(self) -> tuple[bool, float | None]:
        ...
        if avg_vol <= 0:
            return False, None
        dry = all((c.get("volume") or 0) < avg_vol * 0.5 for c in candles[-4:-1])
        if not dry:
            return False, None
        last = candles[-1]
        if (last.get("volume") or 0) > avg_vol * 1.5:
