"""
Smart Money Trap Detection: low-volume breakout + long wick.
Use as confidence reducer / trap flag, not solely as standalone trade engine.
"""
...
class SmartMoneyTrapDetection(BaseStrategy):
    """Flags trap setups (long wick, low volume). Can reduce confidence elsewhere."""
    ...
    def check_entry(self) -> tuple[bool, float | None]:
        ...
        wick = abs(last["high"] - last["close"])
        if body <= 0:
            return False, None
        if wick > body * 2 and (last.get("volume") or 0) < avg_vol:
